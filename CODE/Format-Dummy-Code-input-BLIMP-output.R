...
# Reformat imputed data set for downstream analysis (adapt next snippet to name
# substitution code that follows)
spm_p_imp <- suppressMessages(
  read_csv(
    (here('model4imp1.csv')), col_names = F)[1:3])
names(spm_p_imp) = c('id', 'item', 'response')
spm_p_noMiss <- spm_p_imp %>% 
  spread(item, response) 
names(spm_p_noMiss) <- names_input_orig
...
suppressMessages(
  ...
  spread(item, response) %>% 
  assign(paste0(file_name, '_noMiss'), ., envir = .GlobalEnv)
