...
* `scale_num`: numeric names for the subscales, expressed as an interger range starting with 1 (e.g., `1:8`)
...
```
`input_gathered` has three columns: `id`, `item`, and `response`. We now add a column that provides each item's subscale assignment.
...
We can generate the dummy-code columns with the `recipes` package, which pre-processes input data for modeling or visualization. The following snippet assigns `input_scale` to `dum`, to which the new columns will be appended.

`recipe(~ .)` creates a basic recipe object for further processing, including all columns and rows of the input. `step_dummy()` is the processing step for creating the dummy-code colums for the variable `item`. The variable to be dummy-coded must be coerced to `factor` prior to processing.  `one_hot = T` ensures that dummy columns will be created for ALL values of `item`; `preserve = T` retains the `item` column in the output data object. `prep()` is a required step, this estimates the parameters required to complete processing, based on the data frame specified in the argument `training`. `bake()` is the step required to actually append the new dummy-code columns (the names of which begin with the prefix `item_`) to the input argument.
...
```{r prep_BLIMP_input, echo=40:46, eval=FALSE}
```
...
```{r prep_BLIMP_input, echo=48:51, eval=FALSE}
```
...
* All columns to the right of these four are dummy-code columns, in which each subscale item is dummy-coded relative to the last item in that subscale, such that all items are coded `1` except for the last item in each subscale, which is coded `0`.
