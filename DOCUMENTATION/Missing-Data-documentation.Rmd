...
```
In the next snippet, `dplyr::group_by()` collects the rows with identical values on the person ID variable, so they can retain the "single row" structure of the input table and be summarized in later procedures. `dplyr::arrange()` sorts the rows by values of the person ID variable. 

Within these two `dplyr` functions, the person ID variable is named by the expression `names(input_orig)[1]`. `base::names()` returns a vector of the column names of its argument (a data frame). Recall that in `input_orig`, the person ID variable is the left-most column, or column position `1`. In R, you can use single square brackets (e.g., `[1]`) to subset the column names vector by position.

Note that `names(input_orig)[1]` is wrapped in `rlang::sym()` (preceded by the `!!` operator). `sym()` forces evaluation of its argument as a name, instead of as a string. This is necessary because `dplyr` verbs automatically quote their inputs, and we need R to see `names(input_orig)[1]` as the unquoted name of the person ID variable, not as a quoted string.

`dplyr::mutate()` modifies the `item` column in place, extracting the numeric portion of the value of `item` using `stringr::str_sub()`, and coercing it to a factor by wrapping the expression in `base::as.factor()`.
```{r prep_BLIMP_input, echo=17:19, eval=FALSE}
```
...
###### COMMENTED SNIPPETS
The imputed data file `BLIMP-output1.csv` has no column names, so `read_csv` must be called with the `col_names = F` argument (otherwise, the first row of data will be used as column names). Column names are applied to `temp1` from a character vector using `names()`.
