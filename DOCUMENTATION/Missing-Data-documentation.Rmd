...
* __Multi-factor model__: the test as has a factor structure in which items are assigned to multiple subscales.
...
```
Count the `NA` cells across all items and persons in the input table. `is.na()` returns `TRUE` for each table cell containing `NA`. Because logical `TRUE` is also coded as `1`, `sum()` returns the count of `NA` across all cells. `NA_count` prints this count to the console.
