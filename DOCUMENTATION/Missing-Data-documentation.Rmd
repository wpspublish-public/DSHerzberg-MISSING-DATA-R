...
# The input matrix dummies is all 0s. The series of loops below recode certain
# matrix elements to 1. The first scale has 14 items but the i token only goes
# 1:13. This means that case 14 which corresponds to the last item in scale 1
# for the first case of data, is left at the input value of 0 accross all
# columns. In this way, the first 13 items of scale 1 receive a code of 1 in the
# column corresponding to their item number (captured in the token 1). Thus, all
# items are dummy-coded relative to last item in their scale, which stays colded
# at 0 across all columns.
for(i in 1:13){
...
* Lines that begin with L2 and have only a single quantity in the `Variances` column (e.g., `L2 Intercept (i)`, `L2 scalenum#2`) are _variances_ of a particular scale score. 
...
* `scalenum`: dummy codes reflecting mean differences of the listed scale from scale #1 (expressed roughly on a z-score scale). 
    + In this example, all scales have lower means than scale #1, except for scale #3.
* `s1d1` et al.: dummy codes that capture differences among items with respect to difficulty.
    + Coefficents give mean difference of each item relative to lastsitem on that scale.

### Is scalenum#2 coefficient its mean difference (on a z-score scale) from Intercept (scale #1)
...
* `Tau`: set of z-score cutpoints that slice underlying latent normal scores into four discrete response options (for four-point item rating scale)
