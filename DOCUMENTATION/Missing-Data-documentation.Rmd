...
There is a fundamental tradeoff between precision and efficiency. Because multiple imputation is a computationally intensive process, the use of a precise model with a large data set can can result in an excessively lengthy processing duration.

The recommended approach is to conduct the initial diagnostic run with the `response ~ itemnum` model, which yields the most accurate imputed data sets. (Note: accuracy/precision in this context refers to the magnitude of the standard errors of the imputed values). We can then evaluate whether the resulting processing time is acceptable. If it is not, we can switch to another model.

The three most frequently used regression models are summarized in the next table. These models are documented in detail in the __Appendix__.

| Model                              | Precision  | Computational Efficiency | Max. missing rate
| ---------------------------------- | ---------- | ------------------------ | -----------------
...
On the `MODEL` line in the example script, we specify the `response ~ itemnum` model, in which `response` is regressed on `itemnum`, the dummy-coded item-number variable (recall that dummy code variables were created by listing `itemnum` on the `NOMINAL` command line). 
...
This section provides a more detailed explanation of three commonly used regression models. As noted previously, the recommended approach is to conduct the initial diagnostic run with the `response ~ itemnum` model, which yields the most accurate imputed data sets.
...
When the rate of missingness is small (< 2%), we can use the `response ~` model. This is a _null_ model, so-called because it includes no predictor variables. It is analogous to a factor-analytic model in which all items load on a single latent variable, with identical factor loadings. This common factor loading essentially aggregates the difficulty values of all items into a single value. Thus, this model diverges from the actual state of affairs in which items DO vary in difficulty.

Instead, the `response ~` model represents person differences with single factor score that varies across persons, but does not take into account the varying difficulties of items. This leads to larger standard errors in the estimates of missing values (i.e., a decrement in precision). Here we are trading precision for efficiency. When we use the `response ~` model, BLIMP runs converge quickly, even with vary large data sets.

With tests that have a subscale structure, we can employ the `response ~ scalenum | scalenum`, which incorporates subscale score as a random predictor of item score This model is a useful compromise between the more accurate, computationally intensive`response ~ itemnum` model, and the less accurate, computationally efficient `response ~ ` model.
