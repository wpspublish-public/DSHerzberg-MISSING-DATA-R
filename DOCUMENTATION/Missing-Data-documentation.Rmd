...
[^longnote]: The natural question at this point is, "what constitutes a 'large' data set and an 'excessively lengthy' processing duration?" Unsurprisingly, the answer is, "it depends". As an example, we processed an input data set with about 550 columns (items) and about 1600 rows (persons), with < 2% missingness. We conducted a diagnostic run using the `score ~ itemnum` model with `BURN` set to 5000 iterations. We ran this analysis on a dedicated Windows CPU, enhanced with multiple processors and extra memory. The run lasted more than 7 days.
    ...
    If we need quicker results, and the rate of missingness is lower, we can use more efficient models. When we run the same large input file with the `score ~` model, the analysis converges in less than an hour. Because the rate of missingness is < 2%, we can proceed safely with the latter model.
...
* A _random_ slope _may vary_ across persons.
  + A random slope is not a constant, but rather is a distributed variable. BLIMP estimates the mean and variance of the distribution of random slopes, and these parameters summarize the extent to which the slopes differs between persons.
...
The`score ~ scalenum | scalenum` model is thus more precise than the `score ~ `, which includes neither item difficulties nor subscales scores as predictors of Y~_ip_~.
