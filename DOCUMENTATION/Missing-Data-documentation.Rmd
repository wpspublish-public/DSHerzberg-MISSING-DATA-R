...
In a dramatically oversimplified explanation, BLIMP accomplishes the imputation of missing values by estimating a series of multiple regression models on the input data. The regression equations are then used to predict missing values.
...
What differentiates the scripts for Steps 1 and 2 is the activation/deactivation of the `OPTIONS` and `SAVE` lines (commands can be toggled off by beginning the command line with `#`).
...
Let's now consider an example script for a multi-factor test with 114 items and eight subscales. The example below has a typical set of token substitutions.
...
###### COMMENTED SNIPPETS
In order to run as written, the `DATA` command must name a file located in the same folder/directory as the BLIMP script. If that is not the case, you would need to specify a file path on `DATA`. Because the BLIMP input file has no column names, the `VARIABLE` command is used to name the columns. The first four names are applied, respectively to the four left-most columns on the input file: person ID, item number, item response, and item-scale assignment. The names to the right of `scalenum` are applied to the dummy code columns (d) for each of the eight scales (s). Thus, for example, `s2-d16` names the dummy code column for the 16th item of the second scale. The expression `s1d1-s1d14`, for example, is expanded by BLIMP to yield 14 numerically sequenced names (i.e., `s1d1`, `s1d2`, `s1d13`, etc.)
