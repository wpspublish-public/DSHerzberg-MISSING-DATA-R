...
```
`dplyr::group_by(id)` collects the rows with identical values of `id`, so they can retain the "single row" structure of the input table and be summarized in later procedures. `dplyr::arrange()` sorts the data by `id`. Note the use of `rlang::sym()` (preceded by the `!!` operator) to force the evaluation of `id` as a name, instead of as a string. This is necessary because `dplyr` verbs automatically quote their inputs, and we need R to see `id` as the name of an object in the global environment, not as a quoted string.

`dplyr::mutate()` modifies the `item` column in place, extracting the numeric portion of the value of `item` using `stringr::str_sub()`, and coercing it to a factor by wrapping the expression in `as.factor()`.
