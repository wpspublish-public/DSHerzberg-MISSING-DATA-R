...
When the rate of missingness is small (< 5%), we can use the `response ~` model. This is a _null_ model, so called because it includes no predictor variables. It is analogous to a factor-analysis model in which all items load on a single latent variable, with identical factor loadings. This factor loading essentially aggregates the difficulty values of all items into a single value. Thus the actual state of affairs in which items vary in difficulty is not represented in this model. Instead, it represents person differences by the single factor score that varies across persons, but not across items. With this model, BLIMP runs will converge quickly, even with vary large data sets.

With tests that have a subscale structure, we can employ a model that represents subscale score as a random predictor of item response. This model represents a useful compromise between the more accurate, computationally intensive`response ~ itemnum` model, and the less accurate, computationally efficient `response ~ ` model.
...
Once subscale assignment is coded in this matter, we can use the `response ~  scalenum | scalenum` model to incorporate subscale score as a random predictor of item response. Here are select lines from a BLIMP script that includes this model.
